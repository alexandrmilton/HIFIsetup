...
export type CurrentProfile = { id: string; email: string | null; displayName: string | null; avatarPath: string | null; bio: string | null; isAdmin: boolean; isModerator: boolean };
...
export async function getCurrentProfile(): Promise<CurrentProfile | null> {
  ...
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;
  const { data: profile } = await supabase.from("profiles").select("display_name, avatar_path, bio, is_admin, is_moderator").eq("id", user.id).maybeSingle();
  return { id: user.id, email: user.email ?? null, displayName: profile?.display_name ?? null, avatarPath: profile?.avatar_path ?? null, bio: profile?.bio ?? null, isAdmin: profile?.is_admin ?? false, isModerator: profile?.is_moderator ?? false };
}
