...
type DatabaseSetup = {
  ...
  setup_images?: { path: string; position: number }[] | null;
};
...
type RpcSetup = {
  ...
  categories: string[]; components: RpcComponent[]; comments: RpcComment[]; like_count: number; images?: string[] | null;
  room_size: string | null; has_acoustic_treatment: boolean | null; acoustic_notes: string | null; listening_notes: string | null; budget_range: string | null;
  created_at: string | null; updated_at: string | null;
};
...
const selectSetup = "slug, title, city, country, description, cover_path, is_published, moderation_status, owner_id, created_at, updated_at, room_size, has_acoustic_treatment, acoustic_notes, listening_notes, budget_range, profiles!setups_owner_id_fkey(display_name), setup_components(position, is_extra, components(id, brand, model, category, origin, image_url)), setup_categories(categories(id, name)), setup_likes(count), setup_images(path, position)";
