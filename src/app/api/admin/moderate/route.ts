...
export async function POST(request: Request) {
  ...
  if (!body.slug || !body.status || !allowed.has(body.status)) return NextResponse.json({ error: t.errors.invalidRequest }, { status: 400 });

  const supabase = await createClient();
  const { data: claimsData } = await supabase.auth.getClaims();
  const userId = claimsData?.claims?.sub;
  if (!userId) return NextResponse.json({ error: t.errors.signIn }, { status: 401 });
  ...
  if (error) {
    ...
    return NextResponse.json({ error: forbidden ? t.errors.forbidden : t.errors.statusUpdateFailed }, { status: forbidden ? 403 : 500 });
  }
