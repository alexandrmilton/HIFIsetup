...
export async function POST(request: Request) {
  ...
  if (!body.userId || typeof body.isModerator !== "boolean") return NextResponse.json({ error: t.errors.invalidRequest }, { status: 400 });
  ...
  if (error) {
    ...
    return NextResponse.json({ error: forbidden ? t.errors.forbidden : t.errors.roleUpdateFailed }, { status: forbidden ? 403 : 500 });
  }
