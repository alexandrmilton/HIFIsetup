...
export function ComponentPicker({ onAdd, onClose }: { onAdd: (component: AudioComponent) => void; onClose: () => void }) {
  ...
  function pickFromCatalog(component: AudioComponent) { setPicked(component); setSubStep("details"); }
  function startManual() { setPicked(null); setManual({ brand: query, model: "", category: categories[0], origin: "standard" }); setSubStep("details"); }

  function confirm() {
    const component: AudioComponent = picked ?? { id: `new-${crypto.randomUUID()}`, brand: manual.brand.trim(), model: manual.model.trim(), category: manual.category, origin: manual.origin };
    if (!component.brand.trim() || !component.model.trim()) return;
    onAdd(component);
    setSubStep("added");
  }
    ...
    <div className="picker-card">
      ...
      {subStep === "details" && (
        picked ? (
          <div className="picker-details">
            <div className="draft-component"><span className="component-thumb" /><div><b>{picked.brand} {picked.model}</b><small>{picked.category}</small></div><OriginBadge origin={picked.origin} /></div>
            <button className="button button-dark button-small" type="button" onClick={confirm}>Додати до сетапу</button>
          </div>
        ) : (
          <div className="picker-details">
      ...
      {subStep === "added" && (
        <div className="picker-added">
          <p className="form-success">Компонент додано до сетапу.</p>
